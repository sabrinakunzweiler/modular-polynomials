...
def short_isogeny_chain_kummer(kernel, n):
        """
        We compute a (2^n,2^n)-chain.
        The input is expected to be a (2^(n+1),2^(n+1))-kernel.

        Note the output only contains the invariants of the intermediate curves and some auxiliary points. 
        It cannot be used for pushing points through the chain - but for our lifting algorithm.
        """
        ...
        for k in range(n):
            ...
            if k == 0:
                ...
                t0 = time.process_time()
                phi,short_phi = kummer_isogeny(ker, isogeny_chain[-1],lift=True)
                t_compute_kummer_constants = t_compute_kummer_constants + time.process_time() -t0

            # Update the chain of isogenies
            isogeny_chain.append(phi)
            short_isogeny_chain.append(short_phi)
        ...
        return short_isogeny_chain

'''
The remaining methods are not used in our modular polynomials algorithm
'''
