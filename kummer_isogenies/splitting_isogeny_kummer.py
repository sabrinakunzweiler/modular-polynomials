...
def compute_codomain(ker4, phi):
    ...
    codomain = [E1.j_invariant(),E2.j_invariant()]

    return codomain

def splitting_kummer_isogeny(ker4, phi, lift=False):
    """
    input is a 4-torsion kernel lying above the next 2-isogeny, 
    and data of the previous (2,2)-isogeny
    We assume that the next (2,2)-kernel is of the form <((x-alpha),0), x*(x-beta, 0)>
    ...
    ker2 = [new_Tp1,new_Tp2]
    if lift:
        [new_type1_invariants, special_trafo,aux] = find_special_trafo(type1_invariants, ker2, Tp1,lift=lift)
    ...
    if lift:
        return [invariants, matrices, trafos, codomain], [codomain,aux]
    else:
...
def evaluate_splitting_map(P, type1_invariants):
    ...
    P1x = E * (A*B - 2*A - 2*B) * (X0 + X2)**2 + (4*E)*(X0 + X2)*X1 + 4*X3*(X0 - X1 + X2)
    P1z = E * (A - 2)*(B - 2)* (X0 - X2)**2

    P2x = E * (A*B + 2*A + 2*B) * (X0 + X2)**2 + (4*E)*(X0 + X2)*X1 + 4*X3*(X0 - X1 + X2)
    P2z = E * (A + 2)*(B + 2)* (X0 - X2)**2
