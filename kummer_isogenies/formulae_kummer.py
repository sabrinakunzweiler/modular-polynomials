def find_special_trafo(invariants, kernel, point4,lift=False):
    #we assume that kernel = <((x-alpha),0), x*(x-beta, 0)>,
    #point4 = [x0,x1,x2,x3] with 2*points4 = (x-alpha,0)
    [A,B,C,E] = invariants
    alpha = kernel[0][2]/kernel[0][1]
    beta = kernel[1][1]/kernel[1][0]
    gamma = B - beta

    #symmteric polynomials and auxiliary symmetrics
    s2 = point4[1]/point4[0] - 2*alpha
    s4 = point4[2]/point4[0] - (s2+alpha)*alpha
    t2 = 5*alpha - A - B
    t4 = 2*alpha*(3*alpha-2*B) - 3 + A*B + C
    prod = alpha*(alpha-beta)
    scalar = ((s4+(s2**2-t4)/2)*prod + (s4 - prod)**2)/(prod*(2*s2+t2) + (s4 - prod)*(s2 + 2*alpha - beta))
    ...
    newC = (A - 2*alpha)*(gamma - alpha)*scalar_inv**2 #betanew*gammanew
    newE = E*scalar
    ...
    if lift:
        return [[newA,newB,newC,newE], [a00,a10,a11,a20,a21,a30,a31,a32], [alpha,beta,scalar]]
    else:
...
def invariant_type1_type2(type1_invariants):
    ...
    term = (E*A - (E + E))/16
    return [(A+2)/(A-2), 2*term*(1-C),term*(-B-C-1), term*(B-C-1)]
...
def Richelot_type1_invariants(type1_invariants):
    ...
    [A,B,C,E] = type1_invariants
    deltainv = E/(4*(C-1))
    return [C, E/2 , deltainv*(A*C - B), deltainv*(B - A)]
...
def Richelot_type2_matrix(type2_invariants):
    ...
    M1 = [AB, -2*(AC+AE+C), AB + B, 0  , 2 *(AE + C)*(C+E)/B, -2 *(AE + C + E),  1 ,  B, 0 , 0 ]
    M2 = [AC,  -AB,  0 , 0 , AE, 0  ,0  , -AE + C - E, 1  , 0 ]
    M3 = [A*(A*(4 *E**2 -B2) - B2), 0 , 4 *A*(-B2  + 2*C*E), 4*AE,  A*B2 , 0 , 0 , -B2*(A+1) + 4*C**2 , 4*C, 1 ]
    return [M0,M1,M2,M3]
...
def Richelot_type1_matrix(type1_invariants):
    ...
    BC = B*C
    t1 = AC - BC
    term = (2 *AC-B*(C+1 ))*(AC + A - 2 *B)  + (C+1 )*(C-1 )**2 
    
    
    L0 = [AC-BC, 0 , (-AC + B) + (-AC + B),  delta, 0 ,  C-1 , 0 , A-B, 0 , 0 ] # div 4
    L1 = [-C*(C-1), 0 , (-A*B + C + 1 )*(C-1), 0 , 0 , 0 , delta, -C+1, 0 , 0 ] #div 4
    L2 = [C*(-AC + B), C*(C-1), AC+AC-BC-BC,  0 ,  0 , 0 , 0 , -AC + B, delta, 0 ] # div 4
    L3 = [deltainv*C*term, C*E*(B - AC), -4 *deltainv*((AC-B)*(AC+AC-BC-BC) + C*(C-1)**2 ),  AC-BC , C*E*(C-1)/2  , (AC-BC)*E, 0  ,  deltainv*term, B - AC, delta/2 ]
