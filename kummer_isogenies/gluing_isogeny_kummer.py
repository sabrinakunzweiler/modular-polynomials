...
def find_elliptic_transformation(kernel2,kernel4,lift=False):
    """
    ...
    E1 = kernel2[0][0].scheme()
    E2 = kernel2[0][1].scheme()
    ...
    if lift == False:
        ...
        return [[[A,B,1,E]], [[a_r,b_r,e_r],[a_s,b_s,e_s]],[[r1,r2,r3],[s1,s2,s3]]]
...
def type1_add(type1_invariants, D1,D2):
    ...
    b_comp = b + a*(coef1/den*x+coef0/den)
...
def JacToKum(P, type1_invariants):
    ...
    a = P[0]
    b = P[1]
    y1y2 = b[0]**2*a[2]-b[0]*b[1]*a[1] + b[1]**2*a[0]
    ...
    phi = E*(X0**2*(X1 - 2*(A + B)*X2) + (A*B + 2)*X0*X1*X2 + (-2*(A+B)*X0 + X1)*X2**2)    
    X3 = (phi - 2*y1y2*X0**2)/(X1**2-4*X0*X2)
...
def gluing_kummer_isogeny(kernel4,lift=False):
    ...
    kernel2 = [[double_montgomery(P1,1),double_montgomery(P2,1)],[double_montgomery(Q1,1),double_montgomery(Q2,1)]]

    if lift:
        [invariants,trafos, aux] = find_elliptic_transformation(kernel2,kernel4,lift=True)
        aux_lift = [invariants[0],aux]
        return [invariants,trafos], aux_lift
    else:
        return find_elliptic_transformation(kernel2,kernel4)
